...
// GETTING ANTIBIOTICS DRUG
const antibiotics = drugs.filter(drug => drug.category === "Antibiotic");
console.log(antibiotics)

// RETURNING ARRAY OF DRUG NAMES IN LOWER CASE
const lowerCase = drugs.map(drug => drug.name.toLowerCase());
console.log(lowerCase)

// ACCEPTS CATEGORY AND RETURN DRUGS UNDER THAT CATEGORY
function getByCategory(category){
    return drugs.filter(drug => drug.category=category);
}
console.log(getByCategory("Antacid"))

// DRUG NAME AND ITS MANUFACTURER
drugs.forEach(drug => {
    console.log(`${drug.name} ${drug.manufacturer}`);
});

// DRUGS REQUIRING PRESCRIPTION
const getPrescription = drugs.filter(drug => drug.isPrescriptionOnly);
console.log(getPrescription)

// DRUG NAME AND DOSAGE
const newListOfArray = drugs.map(drug => ` Drug: ${drug.name} ${drug.dosageMg}mg `)
console.log(newListOfArray)

// STOCKS BELOW 50
function getStockBelow50(){
    return drugs.filter(drug => drug.stock<50);
}
console.log(getStockBelow50())

// MANUFACTURE NAME AND HOW MANY DRUGS ARE FROM THE COMPANY
const manufacturerAndStock = drugs.map(drug => ` Drug: ${drug.manufacturer} ${drug.stock}pieces `)
console.log(manufacturerAndStock)

// DRUGS WHICH DOES NOT REQUIRE PRESCRIPTION
